...
public class UserDAOFirebaseImpl implements UserDAO{
    ...
    @Override
    public long addUser_DB(User_DB user) {
        final long[] result = {-1};
        myRef.push().setValue(user, new DatabaseReference.CompletionListener() {
            @Override
            public void onComplete(DatabaseError error, DatabaseReference ref) {
        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot snapshot) {
                for (DataSnapshot data: snapshot.getChildren()) {
                    ...
                    user.setWeightGoal(data.child("weightGoal").getValue(Integer.class));
    ...
    public int updateUser(String uID, String toUpdate, String newValue) {
        if (toUpdate.equals("weight") || toUpdate.equals("weightGoal")) {
